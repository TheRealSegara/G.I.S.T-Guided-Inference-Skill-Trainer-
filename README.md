...
- **Best-effort rate limiting**: a per-instance in-memory limiter (`RATE_LIMIT_MAX_REQUESTS`, default 10/minute/IP — a cheap first layer against obvious abuse, not a precise model of Groq's shared token budget, which normal single-student play can approach without anything being wrong). Since serverless instances are short-lived and not shared, this isn't a global guarantee, it deters casual abuse of a warm instance, not a determined attacker. The real global backstop is the Vercel Firewall rule described below.
