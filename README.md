...
2. In the Supabase dashboard, open **SQL Editor → New query**, paste the entire contents of `supabase/schema.sql` from this repo, and run it. This creates three tables (`students`, `sessions`, `session_words`) with Row Level Security enabled and **no policies** for the anon/authenticated roles — the anon key alone grants zero access.
